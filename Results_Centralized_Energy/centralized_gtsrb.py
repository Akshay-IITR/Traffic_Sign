...
def train_model(model, train_loader, test_loader, device, epochs, lr, save_dir, model_name):
    ...
    # -----------------------------
    # Energy & Time Tracking START
    # -----------------------------
    tracker = EmissionsTracker(project_name=f"centralized_{model_name}", measure_power_secs=1, log_level="error")
    tracker.start()
    ...
    energy_joules = energy_kwh * 3.6e6  # kWh → Joules
    energy_kj = energy_joules / 1000

    # Save energy & time
    energy_log = {"Model": model_name, "Training_Time_sec": round(training_time_sec, 2), "Energy_kWh": round(energy_kwh, 6), "Energy_KJ": round(energy_kj, 3)}

    with open(os.path.join(save_dir, f"{model_name}_energy_time.json"), "w") as f:
        json.dump(energy_log, f, indent=4)

    print(f"{model_name} | Training Time: {training_time_sec:.2f}s | Energy: {energy_kj:.3f} KJ")
    return checkpoint_path, best_acc, training_time_sec, energy_kj
...
def main():
    ...
    for model_name in args.models:
        ...
        checkpoint_path, best_acc, train_time, energy_kj = train_model(model, train_loader, test_loader, args.device, args.epochs, args.lr, args.save_dir, model_name)
        ...
        results.append({"Model": model_name, "Best Accuracy": round(best_acc, 4), "Final Accuracy": round(acc, 4), "Precision": round(precision, 4),
                        "Recall": round(recall, 4), "F1-Score": round(f1, 4), "Training Time (s)": round(train_time, 2), "Energy (KJ)": round(energy_kj, 3)})
